...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None, name_field="name"):
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", name_field="name"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--synonyms', action='store_true', help='query the name.synonyms field')
    ...
    for line in fileinput.input():
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, name_field=name_field)
