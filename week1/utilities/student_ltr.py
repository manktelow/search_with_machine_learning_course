...
def extract_logged_features(hits, query_id):
    ...
    for (idx, hit) in enumerate(hits):
        ...
        feature_results["sku"].append(int(hit['_source']['sku'][0])) # assume only one sku

        for (_idx, feature) in enumerate(hit['fields']['_ltrlog'][0]['log_entry']):
            if feature['name'] not in feature_results:
                feature_results[feature['name']] = []

            try:
                feature_results[feature['name']].append(int(feature['value']))
            except KeyError:
    ...
    for key in feature_results.keys():
        frame_types[key] = 'int64'

    return frame.astype(frame_types)